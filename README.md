...
The canonical transport artifact is .npk: a tar archive compressed with zstd. The manifest is deliberately format-neutral. runtime_requires records discovered runtime capabilities such as ELF DT_NEEDED libraries, while provides records capabilities supplied by a package. Installation checks runtime_requires against installed provides before extraction. System-library capability providers are a future adapter. The release-event command uses the official Rust Nostr library for event IDs, tags, key handling, and Schnorr signatures while preserving this local package lifecycle.
